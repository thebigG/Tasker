...
void dispatch_proc(uiohook_event * const event) {
	switch (event->type) {
		...
		if(current_mode == IOHOOK_KEYBOARD_MODE || current_mode == IOHOOK_KEYBOARD_AND_MOUSE_MODE)
		{
			printf("k\n");
		}
		break;
		...
		if(current_mode == IOHOOK_MOUSE_MODE || current_mode == IOHOOK_KEYBOARD_AND_MOUSE_MODE)
		{
			printf("m\n");
		}
		break;
