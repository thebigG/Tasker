...
: QWidget() {
    ...
    labelPalette.setColor(QPalette::ColorRole::Foreground, productiveBarSet.color());
    ...
    labelPalette.setColor(QPalette::ColorRole::Foreground, unproductiveBarSet.color());
    unproductiveTimeAvgLabel.setPalette(labelPalette);
    unproductiveTimeAvgLabel.setFont(chartFont);
    static_cast<QGridLayout *>(detailsWidget.layout())->addWidget(&productiveTimeAvgLabel, 0, 0);
    static_cast<QGridLayout *>(detailsWidget.layout())->addWidget(&unproductiveTimeAvgLabel, 0, 1);
...
void CommitmentSnaphot::update(udata::TimeWindow &currentWindow,
     ...
     */
    if (type == udata::CommitmentType::WEEKLY || type == udata::CommitmentType::EVERDAY) {
    ...
    productiveRatio =
        productiveBarSet.sum() / productiveBarSet.sum() + unproductiveBarSet.sum();
    unproductiveRatio = unproductiveBarSet.sum() / productiveBarSet.sum() +
                        unproductiveBarSet.sum();
    util::formatTime(productibeTimeAvgText, util::minutesToSeconds(productiveTimeAverage),
                     productiveTimeAvgLabelContext);
    productiveTimeAvgLabel.setText(productibeTimeAvgText);
    util::formatTime(unproductibeTimeAvgText, util::minutesToSeconds(unproductiveTimeAverage),
